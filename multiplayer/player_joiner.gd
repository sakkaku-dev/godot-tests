...
func _request_spawned_players():
	...
	print("Player %s requests all player data: %s" % [requester, joined_players.keys()])

@rpc("call_local", "reliable", "any_peer")
func _join_player(input: String):
	if disabled:
		print("Player joining is disabled")
		return

	var id = multiplayer.get_remote_sender_id()
	print("Received join request for %s from %s" % [input, id])
	
	var unique_id = "%s_%s" % [id, input]
	if unique_id in joined_players:
		print("Player %s already exists" % unique_id)
		return
